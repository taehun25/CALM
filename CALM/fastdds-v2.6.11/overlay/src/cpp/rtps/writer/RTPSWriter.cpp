...
{
    {
        ...
        const char* calm_enabled = std::getenv("FASTDDS_CALM_ENABLED");
        const bool use_calm = calm_enabled != nullptr &&
                (0 != std::strcmp(calm_enabled, "0") &&
                0 != std::strcmp(calm_enabled, "false") &&
                0 != std::strcmp(calm_enabled, "off"));
        if (RELIABLE == att.endpoint.reliabilityKind)
        {
            const char* raw_max_size = std::getenv("FASTDDS_RTPS_MAX_MESSAGE_SIZE");
            if (raw_max_size == nullptr && use_calm)
            {
                raw_max_size = std::getenv("FASTDDS_CALM_MAX_RTPS_MESSAGE_SIZE");
            }

            // Preserve the existing CALM default while allowing packetization-only A/B tests.
            const unsigned long configured_max_size =
                    raw_max_size == nullptr && use_calm ?
                    1400ul :
                    (raw_max_size == nullptr ? 0ul : std::strtoul(raw_max_size, nullptr, 10));
            if (configured_max_size >= 512ul && configured_max_size <= 65500ul)
            {
                max_output_message_size_ = (std::min)(
                    max_output_message_size_, static_cast<uint32_t>(configured_max_size));
            }
        }
    }
