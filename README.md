...
- $N_{s,r}$: 샘플 $s$를 reader $r$에게 재전송한 횟수
...
- $\Delta \rho_r \leq 0$: backlog가 안정 또는 감소 중이므로 $B_r$을 조금씩 증가시킨다.
- $N_{s,r}$이 큰 샘플은 재시도 간격을 늘려 반복 실패 샘플이 채널을 계속 두드리지 않게 한다.
...
CALM은 평균 부하가 채널 용량 이하인 $\lambda \leq \mu$ 상황에서 다음을 목표로 합니다.
...
- convergence time $T_{\mathrm{conv}}$
