        ...
        const WriterAttributes& att)
{
    ...
    calm_pacing_event_ = new TimedEvent(
        pimpl->getEventResource(),
        [&]() -> bool
        {
            return perform_calm_pacing();
        },
        1.0);
...
StatefulWriter::~StatefulWriter()
{
    ...
    if (calm_pacing_event_ != nullptr)
    {
        delete(calm_pacing_event_);
        calm_pacing_event_ = nullptr;
    }
        ...
        const std::chrono::time_point<std::chrono::steady_clock>& max_blocking_time)
{
    ...
    if (!matched_remote_readers_.empty() || !matched_datasharing_readers_.empty() || !matched_local_readers_.empty())
    {
        ...
        bool calm_held_new_added = false;
        for_matched_readers(matched_local_readers_, matched_datasharing_readers_, matched_remote_readers_,
                [this, &should_be_sent, &calm_held_new_added, &change, &max_blocking_time](ReaderProxy* reader)
                {
                    ...
                    if (m_pushMode || !reader->is_reliable() || reader->is_local_reader())
                    {
                        ...
                        if (is_revelant && reader->is_remote_and_reliable() && reader->calm_should_hold_new())
                        {
                            changeForReader.calm_hold_new();
                            calm_held_new_added = true;
                        }
                        else
                        {
                            should_be_sent |= is_revelant;
                            if (is_revelant && reader->is_remote_and_reliable() && reader->calm_enabled())
                            {
                                changeForReader.calm_mark_new_queued();
                            }
                        }
                    }
                    else
        ...
        if (calm_held_new_added && calm_pacing_event_ != nullptr)
        {
            for (ReaderProxy* reader : matched_remote_readers_)
            {
                if (reader->calm_held_new_size() > 0)
                {
                    reader->calm_pacing_active(true);
                }
            }
            calm_pacing_event_->restart_timer();
        }
        ...
        CacheChange_t* a_change)
{
    ...
    for_matched_readers(matched_local_readers_, matched_datasharing_readers_, matched_remote_readers_,
            [this, sequence_number](ReaderProxy* reader)
            {
                reader->change_has_been_removed(sequence_number);
                if (reader->calm_backlog_size() == 0 && reader->calm_held_new_size() > 0)
                {
                    reader->calm_pacing_active(true);
                    calm_pacing_event_->restart_timer();
                }
                            ...
                            if (min_unsent_fragment != n_fragments + 1)
                            {
                                if (group.add_data_frag(*change, min_unsent_fragment, inline_qos))
                                {
                                            ...
                                            ++remote_reader)
                                    {
                                        ...
                                        {
                                            (*remote_reader)->storm_observe_transport_repair(
                                                change->sequenceNumber, min_unsent_fragment);
...
void StatefulWriter::perform_nack_response()
{
    ...
    bool has_pending_requested_changes = false;
    for (ReaderProxy* reader : matched_remote_readers_)
    {
        // T_NR has expired and REQUESTED changes are now known.  Let CALM
        // inspect the Reader-local repair debt before stock Fast DDS releases
        // the complete requested set to the FlowController.
        reader->calm_evaluate_before_tnr();
        if (!reader->calm_enabled() || !reader->calm_controls_repair())
        {
            const uint64_t requested_bytes = reader->calm_requested_bytes();
            const uint32_t requested_changes = reader->calm_requested_size();
            uint64_t accepted_bytes = 0;
            uint64_t rejected_bytes = 0;
            uint32_t accepted_changes = 0;
            uint32_t rejected_changes = 0;
            changes_to_resend += reader->perform_acknack_response(
                [&](ChangeForReader_t& change)
                {
                    assert(nullptr != change.getChange());
                    const uint64_t change_bytes = reader->calm_change_requested_bytes(change);
                    if (flow_controller_->add_old_sample(this, change.getChange()))
                    {
                        ++accepted_changes;
                        accepted_bytes += change_bytes;
                    }
                    else
                    {
                        ++rejected_changes;
                        rejected_bytes += change_bytes;
                    }
                });
            reader->calm_observe_default_release(requested_bytes);
            reader->storm_observe_tnr_release(
                requested_bytes, requested_changes,
                accepted_bytes, accepted_changes,
                rejected_bytes, rejected_changes);
            continue;
        }

        if (reader->calm_requested_size() == 0)
        {
            const bool has_work = reader->calm_has_pacing_work();
            reader->calm_pacing_active(has_work);
            has_pending_requested_changes |= has_work;
            continue;
        }

        const uint64_t calm_budget = reader->calm_update_budget();
        const uint64_t requested_bytes = reader->calm_requested_bytes();
        const uint32_t requested_changes = reader->calm_requested_size();
        ...
        uint32_t accepted_changes = 0;
        uint32_t rejected_changes = 0;
        if (reader->calm_can_release_repair())
        {
            changes_to_resend += reader->perform_acknack_response_limited(calm_budget,
                            [&](ChangeForReader_t& change)
                            {
                                assert(nullptr != change.getChange());
                                const uint64_t change_bytes =
                                        reader->calm_change_requested_bytes(change);
                                if (flow_controller_->add_old_sample(this, change.getChange()))
                                {
                                    ++accepted_changes;
                                    accepted_bytes += change_bytes;
        ...
        reader->storm_observe_tnr_release(
            ...
            accepted_bytes, accepted_changes,
            rejected_bytes, rejected_changes);
        reader->calm_log_release(
            released_bytes > 0 ? "initial_release" : "initial_deferred",
            released_bytes);
        const bool reader_has_pending = reader->calm_has_pacing_work();
        reader->calm_pacing_active(reader_has_pending);
        has_pending_requested_changes |= reader_has_pending;
    }

    if (has_pending_requested_changes && calm_pacing_event_ != nullptr)
    {
        calm_pacing_event_->restart_timer();
    }
...
bool StatefulWriter::perform_calm_pacing()
{
    std::unique_lock<RecursiveTimedMutex> lock(mp_mutex);
    ...
    for (ReaderProxy* reader : matched_remote_readers_)
    {
        if (!reader->calm_pacing_active())
        {
            continue;
        }

        uint64_t repair_released_bytes = 0;
        uint64_t new_released_bytes = 0;
        const uint64_t total_budget = reader->calm_total_release_budget_bytes();
        const uint64_t requested_bytes = reader->calm_requested_bytes();
        const uint32_t requested_changes = reader->calm_requested_size();
        uint64_t accepted_bytes = 0;
        uint64_t rejected_bytes = 0;
        uint32_t accepted_changes = 0;
        uint32_t rejected_changes = 0;
        const char* phase = "pacing_wait";
        if (reader->calm_can_release_repair())
        {
            changes_to_resend += reader->perform_acknack_response_limited(
                std::min(reader->calm_budget_bytes(), total_budget),
                [&](ChangeForReader_t& change)
                {
                    assert(nullptr != change.getChange());
                    const uint64_t change_bytes = reader->calm_change_requested_bytes(change);
            ...
            phase = "pacing_repair_release";
            reader->storm_observe_tnr_release(
                requested_bytes, requested_changes,
                accepted_bytes, accepted_changes,
                rejected_bytes, rejected_changes);
        }
        const bool alongside_repair = repair_released_bytes > 0;
        if (repair_released_bytes < total_budget &&
                reader->calm_requested_size() == 0 &&
                reader->calm_can_release_held_new(alongside_repair))
        {
            reader->calm_release_held_new(
                total_budget - repair_released_bytes,
                [&](ChangeForReader_t& change)
                {
                    assert(nullptr != change.getChange());
                    // A held-new change already lives in WriterHistory. Queue it
                    // behind the repair batch as deferred historical work so
                    // FlowQueue's new-before-old preference cannot invert
                    // CALM's repair-first shared-budget order.
                    flow_controller_->add_old_sample(this, change.getChange());
                },
                new_released_bytes,
                alongside_repair);
            phase = alongside_repair ?
                    "pacing_shared_release" : "pacing_new_release";
        }
        const uint64_t released_bytes = repair_released_bytes + new_released_bytes;
        if (released_bytes > 0)
        {
            reader->calm_note_shared_release(released_bytes, repair_released_bytes);
            reader->calm_log_release(phase, released_bytes);
        }
    ...
    lock.unlock();
    on_resent_data(changes_to_resend);
    return has_pending_requested_changes;
}
                    ...
                    [&](ReaderProxy* remote_reader)
                    {
                        if (remote_reader->guid() == reader_guid)
                        {
                            if (remote_reader->check_and_set_acknack_count(ack_count))
                            {
                                ...
                                if (remote_reader->calm_backlog_size() == 0 &&
                                        remote_reader->calm_held_new_size() > 0)
                                {
                                    remote_reader->calm_pacing_active(true);
                                    calm_pacing_event_->restart_timer();
                                }
